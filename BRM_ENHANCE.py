...
from PyQt5.QtWidgets import (
    ...
    QTreeWidget, QTreeWidgetItem, QButtonGroup, QRadioButton
)
...
def get_op_type_from_sql(sql_text: str) -> str:
    """
    Returns 'INSERT', 'DELETE', 'UPDATE', 'SELECT', or 'OTHER' by examining
    the start of the SQL statement (case-insensitive).
    """
...
def setup_in_memory_db():
    ...
    CREATE TABLE IF NOT EXISTS BRM_RULES(
        ...
        CLUSTER_NAME TEXT,           -- We keep the column in DB but won't show in CRUD editor
...
def send_email(to_addr, subject, body):
    # Example stub, replace with real SMTP details
...
def extract_tables(sql_text):
    ...
    This can be improved with a real SQL parser.
    """
...
##############################################################################
# PART 3: CRUD FUNCTIONS FOR BRM_RULES
##############################################################################
...
def add_rule(conn, rule_data, created_by, user_group):
    """
    Insert a new rule. 'created_by' is automatically the logged-in user (no UI).
    """
    c = conn.cursor()
    c.execute("""
...
##############################################################################
# PART 4: BFS-BASED LINEAGE VISUALIZATION
##############################################################################
...
class EnhancedLineageGraphWidget(QtWidgets.QGraphicsView):
    ...
    def edit_rule(self, rule_id):
        ...
        # We do not have the user_group or user_id here. We'll just pass "Admin" for demonstration.
...
##############################################################################
# PART 5: RULE EDITOR (Two-grid, no CreatedBy, no ClusterName, auto OperationType)
##############################################################################

class RuleEditorDialog(QDialog):
    """
    Uses a 2-column layout.
    'CREATED_BY' is auto from the logged_in_user, no UI field.
    'CLUSTER_NAME' is not in this form.
    'OPERATION_TYPE' is auto detected from get_op_type_from_sql(...).
    """
...
##############################################################################
# PART 7: AuditLogViewer, SearchRuleDialog
##############################################################################
...
##############################################################################
# PART 8: RULE DASHBOARD (Now with "Rule Analytics" popup)
##############################################################################
...
##############################################################################
# PART 9: BACKUP/RESTORE (BUSINESS GROUPS) + CUSTOM GROUPS
##############################################################################
...
##############################################################################
# [CUSTOM GROUP ENHANCEMENT] backup_custom_group, restore_custom_group, etc.
##############################################################################
...
##############################################################################
# PART 10: UI TABS FOR GROUP MGMT, CUSTOM GROUPS, ETC.
##############################################################################

# GroupManagementTab, CustomRuleGroupTab, etc. are the same from prior versions
# with the same logic for backups/restores.
# We just keep them to ensure no functional loss.

class GroupManagementTab(QWidget):
    """
    Only visible to Admin. Manage business groups & membership, etc.
    """
    ...
    # ... [Methods: load_data, load_groups, etc.] same as before ...
    def load_data(self):
        self.load_groups()
        self.load_users()
...
# CustomRuleGroupTab omitted for brevity, but typically same as prior version
...
class LoginDialog(QDialog):
    """
    Simple login. On success, sets self.user_id, self.user_group, and accepts.
    """
    def __init__(self, connection, parent=None):
        super().__init__(parent)
        self.connection = connection
...
class BusinessRuleManagementTab(QWidget):
    """
    Provides the top-level CRUD (Add/Update/Deactivate/Delete),
    plus search rules (via main_app), plus audit logs.
    """
    def __init__(self, main_app, connection, user_id, user_group, parent=None):
        super().__init__(parent)
        self.main_app = main_app
    ...
    def on_add_rule(self):
        ...
        # pass the user_group or self.user_id as "logged_in_user"
...
class BRMTool(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("BRM Tool - Full Integrated (UI Enhancements)")
    ...
    def refresh_dashboard(self):
        # if you want periodic refresh logic, put it here
        pass
